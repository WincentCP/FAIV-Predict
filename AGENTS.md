...
- **ml-service** (`ml-service/`) — FastAPI + scikit-learn inference engine. Port **8000**. Endpoints: `/predict`, `/suggest`, `/train`, `/train/{job_id}`, `/instagram/health`, `/sync/now` (Swagger at `/docs`).
