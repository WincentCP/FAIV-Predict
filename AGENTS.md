...
- **Tests:** install `ml-service/requirements-dev.txt`, then run `./venv/bin/python -m pytest` from `ml-service/`. The frontend has no test framework configured; use `npm run lint`, `npx tsc --noEmit`, and `npm run build`.
