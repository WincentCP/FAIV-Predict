...
ALTER TABLE brands ADD COLUMN IF NOT EXISTS instagram_account_id VARCHAR(64);
ALTER TABLE brands ADD COLUMN IF NOT EXISTS profile_summary TEXT;
ALTER TABLE brands ADD COLUMN IF NOT EXISTS timezone VARCHAR(64) DEFAULT 'Asia/Jakarta' NOT NULL;
...
CREATE TABLE IF NOT EXISTS prediction_publications (
    ...
    owner_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
    brand_id UUID REFERENCES brands(id) ON DELETE CASCADE NOT NULL,
    prediction_id UUID REFERENCES predictions(id) ON DELETE RESTRICT NOT NULL,
    post_id UUID REFERENCES posts(id) ON DELETE RESTRICT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_brands_instagram_account_unique
  ON brands(instagram_account_id) WHERE instagram_account_id IS NOT NULL;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'brands_instagram_account_id_check' AND conrelid = 'public.brands'::regclass
  ) THEN
    ALTER TABLE public.brands
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'brands_profile_summary_length_check' AND conrelid = 'public.brands'::regclass
  ) THEN
    ALTER TABLE public.brands
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'brands_timezone_wib_check' AND conrelid = 'public.brands'::regclass
  ) THEN
    ALTER TABLE public.brands
...
GRANT INSERT (owner_id, name, niche, profile_summary, timezone) ON public.brands TO authenticated;
GRANT UPDATE (name, niche, profile_summary, timezone) ON public.brands TO authenticated;
...
DROP POLICY IF EXISTS publication_owner_select ON prediction_publications;
CREATE POLICY publication_owner_select ON prediction_publications FOR SELECT TO authenticated
USING (owner_id = auth.uid());
...
FOR EACH ROW EXECUTE FUNCTION public.prevent_brand_instagram_identity_change();
...
BEGIN
  ...
  FROM public.predictions prediction
  WHERE prediction.id = NEW.prediction_id AND prediction.deleted_at IS NULL;
  IF NOT FOUND OR prediction_owner IS NULL THEN
    RAISE EXCEPTION 'Prediction must be active and owned.' USING ERRCODE = '23514';
  END IF;
  ...
  IF NOT FOUND OR post_source IS DISTINCT FROM 'instagram_graph' OR post_media_id IS NULL THEN
    RAISE EXCEPTION 'Publication post must be verified Instagram Graph media.'
      USING ERRCODE = '23514';
  END IF;
  SELECT brand.owner_id INTO brand_owner FROM public.brands brand
...
FOR EACH ROW EXECUTE FUNCTION public.validate_prediction_publication();
...
BEGIN
  INSERT INTO public.content_lifecycle_events (
    ...
    NEW.owner_id, NEW.brand_id, NEW.prediction_id, 'publication.linked',
    'database_trigger', NEW.linked_by,
    jsonb_build_object(
...
FOR EACH ROW EXECUTE FUNCTION public.audit_prediction_publication_link();
...
BEGIN
  ...
  INSERT INTO public.content_lifecycle_events (
    ...
    NEW.created_by, NEW.brand_id, NEW.id,
...
BEGIN
  ...
  SET last_reconciled_at = timezone('utc'::text, now()),
      outcome_observed_at = COALESCE(
        publication.outcome_observed_at, timezone('utc'::text, now())
      )
...
END
$$;
DROP TRIGGER IF EXISTS prediction_publications_reconcile_outcome ON public.prediction_publications;
CREATE TRIGGER prediction_publications_reconcile_outcome
AFTER INSERT ON public.prediction_publications
FOR EACH ROW EXECUTE FUNCTION public.reconcile_new_prediction_publication();

REVOKE ALL ON FUNCTION public.link_prediction_publication(UUID, UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.link_prediction_publication(UUID, UUID) TO authenticated;
REVOKE ALL ON FUNCTION public.prevent_brand_instagram_identity_change() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.validate_prediction_publication() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.audit_prediction_publication_link() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.validate_prediction_observed_outcome() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.audit_prediction_observed_outcome() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.reconcile_prediction_publication_outcomes(UUID) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.reconcile_new_prediction_publication() FROM PUBLIC, anon, authenticated;
